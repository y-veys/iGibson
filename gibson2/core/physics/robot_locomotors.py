...
class JR2_Kinova(LocomotorRobot):
    ...
    def robot_specific_reset(self):
        ...
        #self.ordered_joints[2].reset_joint_state(np.random.uniform(-13.0*np.pi/24.0, -np.pi/2.0), 0.0)
        #self.ordered_joints[3].reset_joint_state(np.random.uniform(11.0*np.pi/24.0, 13.0*np.pi/24.0), 0.0)
        #self.ordered_joints[4].reset_joint_state(np.random.uniform(11.0*np.pi/24.0, 13.0*np.pi/24.0), 0.0)
...
class JR2_Kinova_Wrist(LocomotorRobot):
    def __init__(self, config):
        self.config = config
        self.wheel_velocity = config.get('wheel_velocity', 0.25)
        self.wheel_dim = 2
        self.arm_velocity = config.get('arm_velocity', 0.05)
        self.arm_dim = 5

        LocomotorRobot.__init__(self,
                                "jr2_urdf/jr2_kinova_wrist_cam.urdf",
                                action_dim=self.wheel_dim + self.arm_dim,
                                scale=config.get("robot_scale", 1.0),
                                is_discrete=config.get("is_discrete", False),
                                control='velocity',
    ...
    def set_up_discrete_action_space(self):
        assert False, "JR2_Kinova_Wrist does not support discrete actions"
    ...
    # initialize JR's arm to almost the same height as the door handle to ease exploration
    def robot_specific_reset(self):
        super(JR2_Kinova_Wrist, self).robot_specific_reset()
        #self.ordered_joints[2].reset_joint_state(np.random.uniform(-3.0*np.pi/4.0, -np.pi/2.0), 0.0)
        #self.ordered_joints[3].reset_joint_state(np.random.uniform(np.pi/4.0, 3.0*np.pi/4.0), 0.0)
        #self.ordered_joints[4].reset_joint_state(np.random.uniform(np.pi/4.0, 3.0*np.pi/4.0), 0.0)
        #self.ordered_joints[2].reset_joint_state(np.random.uniform(-16.0*np.pi/24.0, -np.pi/2.0), 0.0)
    ...
    def load(self):
        ids = super(JR2_Kinova_Wrist, self).load()
        robot_id = self.robot_ids[0]
...
class JR2_Kinova_Head(LocomotorRobot):
    def __init__(self, config):
        self.config = config
        self.wheel_velocity = config.get('wheel_velocity', 0.3)
        self.wheel_dim = 2
    ...
    def set_up_continuous_action_space(self):
        ...
        self.action_low = -self.action_high
        self.action_space = gym.spaces.Box(shape=(self.wheel_dim + self.head_dim + self.arm_dim,),
                                           low=-1.0,
                                           high=1.0,
                                           dtype=np.float32)

    def set_up_discrete_action_space(self):
        assert False, "JR2_Kinova_Head does not support discrete actions"
    ...
    def robot_specific_reset(self):
        ...
        self.ordered_joints[2].reset_joint_state(0.1, 0.0)
